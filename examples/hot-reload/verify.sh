...
# Atomic rewrite: drop ssrf.ip_allowlist while keeping the same listen port.
write_blocked_config() {
  local proxy_port="$1"
  python3 - <<'PY' "$CONFIG" "$proxy_port"
import os
import sys
from pathlib import Path

dst, port = sys.argv[1], sys.argv[2]
text = f"""# Generated by verify.sh — blocked phase (no ssrf.ip_allowlist)
version: 1
mode: balanced
...
health_json() {
  local port="$1"
  local out_file="$2"
  curl -sS -o "$out_file" -w '%{http_code}' "http://127.0.0.1:${port}/health"
}
...
write_blocked_config "$PROXY_PORT"
